...
class TestSmartValidation:
    ...
    def test_valid_files_comprehensive(
        self, shacl_validator, data_loader, test_data_files, verbose_validation
    ):
        ...
        for file_name, file_path in valid_files.items():
            try:
                ...
                conforms_basic, report_basic, text_basic = shacl_validator(
                    data_graph, test_name=f"basic-{file_name}", use_ontology=False
                )
                ...
                conforms_full, report_full, text_full = shacl_validator(
                    data_graph, test_name=f"full-{file_name}", use_ontology=True
                )
                ...
                if not conforms_basic and not conforms_full:
                    ...
                    if verbose_validation:
                        print(
                            f"⚠️  {file_name}: PASSED with ontology (datatype conversion)"
                        )
    ...
    def test_invalid_files_comprehensive(
        ...
        for file_name, file_path in invalid_files.items():
            try:
                ...
                conforms_full, report_full, text_full = shacl_validator(
                    data_graph, test_name=f"invalid-full-{file_name}", use_ontology=True
                )
                ...
                if conforms_basic and conforms_full:
                    ...
                    if verbose_validation:
                        if not conforms_basic:
                            ...
                            print(
                                f"❌ {file_name}: Failed basic validation ({len(violations)} violations)"
                            )
                        if not conforms_full:
                            violations = get_violation_details(report_full)
                            print(
                                f"❌ {file_name}: Failed full validation ({len(violations)} violations)"
                            )
        ...
        if unexpected_passes:
            error_msg = (
                "The following invalid files unexpectedly passed all validation:\n"
            )
    ...
    def test_priority_validation_scenarios(
        ...
        if use_ontology:
            ...
            if verbose_validation:
                if conforms:
                    print(
                        f"⚠️  Priority validation without ontology passed (expected limitation)"
                    )
                else:
    ...
    def test_context_pattern_validation(
        ...
        if not conforms:
            ...
            if "PatternConstraintComponent" not in report_text:
                if verbose_validation:
                    print(
                        f"⚠️  Context validation failed but not due to pattern constraint"
                    )
        else:
            if verbose_validation:
                print(
                    f"⚠️  Context pattern validation unexpectedly passed with ontology={use_ontology}"
                )
...
class TestValidationEdgeCases:
    ...
    def test_missing_required_properties(
        ...
        conforms_basic, report_basic, text_basic = shacl_validator(
            data_graph, test_name="missing-props-basic", use_ontology=False
        )

        conforms_full, report_full, text_full = shacl_validator(
            data_graph, test_name="missing-props-full", use_ontology=True
        )

        # At least one approach should detect missing properties
        if conforms_basic and conforms_full:
            pytest.fail(
                "Missing required properties not detected by either validation approach"
            )
    ...
    def test_sparql_constraints_require_ontology(
        ...
        for file_name in sparql_dependent_files:
            ...
            conforms_basic, _, _ = shacl_validator(
                data_graph, test_name=f"sparql-basic-{file_name}", use_ontology=False
            )

            conforms_full, _, _ = shacl_validator(
                data_graph, test_name=f"sparql-full-{file_name}", use_ontology=True
            )

            results[file_name] = {"basic": conforms_basic, "full": conforms_full}

        if verbose_validation:
            print("SPARQL constraint validation results:")
            for file_name, result in results.items():
                print(
                    f"  {file_name}: basic={not result['basic']}, full={not result['full']}"
                )
        ...
        if verbose_validation:
            print(
                f"SPARQL constraints improved with ontology: {sparql_improvements}/{len(results)}"
            )

...
class TestValidDataFiles:
        ...
        for file_name, file_path in valid_files.items():
            ...
            try:
                ...
                conforms, report_graph, report_text = shacl_validator(
                    data_graph, test_name=f"valid-{file_name}"
                )

                if verbose_validation:
                    print_validation_summary(
                        conforms, report_graph, file_name, verbose=True
                    )
    ...
    def test_specific_valid_file(
        ...
        data_graph = data_loader(file_path)
        conforms, report_graph, report_text = shacl_validator(
            data_graph, test_name=f"specific-valid-{file_name}"
        )
        ...
        assert_validation_passes(
            conforms, report_text, f"Valid file {file_name} should pass validation"
        )
...
class TestInvalidDataFiles:
    ...
    def test_all_invalid_files_fail(
        ...
        for file_name, file_path in invalid_files.items():
            ...
            try:
                data_graph = data_loader(file_path)
                conforms, report_graph, report_text = shacl_validator(
                    data_graph, test_name=f"invalid-{file_name}"
                )
    ...
    def test_specific_invalid_file(
        ...
        data_graph = data_loader(file_path)
        conforms, report_graph, report_text = shacl_validator(
            data_graph, test_name=f"specific-invalid-{file_name}"
        )
        ...
        assert_validation_fails(
            conforms, report_text, f"Invalid file {file_name} should fail validation"
        )
...
class TestRecurrenceValidation:
    ...
    def test_valid_recurrence_file(
        ...
        data_graph = data_loader(file_path)
        conforms, report_graph, report_text = shacl_validator(
            data_graph, test_name="recurrence-valid"
        )
        ...
        assert_validation_passes(
            conforms, report_text, "Valid recurrence action should pass"
        )
    ...
    def test_invalid_recurrence_both_termination(
        ...
        data_graph = data_loader(file_path)
        conforms, report_graph, report_text = shacl_validator(
            data_graph, test_name="recurrence-both-termination"
        )
        ...
        assert_validation_fails(
            conforms, report_text, "Recurrence with both until and count should fail"
        )
...
class TestTemporalValidation:
    ...
    def test_temporal_inconsistency(
        ...
        data_graph = data_loader(file_path)
        conforms, report_graph, report_text = shacl_validator(
            data_graph, test_name="temporal-inconsistency"
        )
        ...
        assert_validation_fails(
            conforms, report_text, "Completion before schedule should fail validation"
        )
...
class TestComplexHierarchy:
    ...
    def test_complete_hierarchy(
        ...
        data_graph = data_loader(file_path)
        conforms, report_graph, report_text = shacl_validator(
            data_graph, test_name="hierarchy-complete"
        )
        ...
        assert_validation_passes(
            conforms, report_text, "Complete hierarchy should pass validation"
        )

...
class TestOntologyConsistency:
    ...
    def test_core_classes_defined(self, ontology_graph):
        ...
        for class_name, cls in core_classes:
            assert (cls, RDF.type, OWL.Class) in ontology_graph, \
                f"Class {class_name} not defined as owl:Class"
    ...
    def test_class_hierarchy_consistency(self, ontology_graph):
        ...
        for name, subclass, superclass in subclass_relationships:
            assert (subclass, RDFS.subClassOf, superclass) in ontology_graph, \
                f"{name} should be subclass of Action"
    ...
    def test_no_syntax_errors(self, ontology_graph):
        ...
        try:
            ...
            assert len(test_graph) == len(ontology_graph), \
...
class TestSHACLShapesConsistency:
    ...
    def test_shape_properties_reference_valid_properties(self, shapes_graph, ontology_graph):
        ...
        for shape in shapes_graph.subjects(RDF.type, sh_ns.NodeShape):
            for sparql_constraint in shapes_graph.objects(shape, sh_ns.sparql):
                ...
                for select_query in shapes_graph.objects(sparql_constraint, sh_ns.select):
        ...
        for prop_path in property_paths:
            is_object_prop = (prop_path, RDF.type, owl_ns.ObjectProperty) in ontology_graph
            is_datatype_prop = (prop_path, RDF.type, owl_ns.DatatypeProperty) in ontology_graph
            is_functional_prop = (prop_path, RDF.type, owl_ns.FunctionalProperty) in ontology_graph
        ...
        if undefined_props:
            ...
            print(f"✅ All {len(property_paths)} shape properties reference valid ontology properties")
    ...
    def test_shapes_have_meaningful_constraints(self, shapes_graph):
        ...
        print(f"✅ Found {shape_count} shapes with {len(constraint_types)} types of constraints")
                ...
                for s, p, o in graph:
                    ...
                    if str(p).startswith(str(actions_ns)):
                        if p not in ontology_properties:
                            undefined_usage.append(f"{file_name}: undefined property {p}")
